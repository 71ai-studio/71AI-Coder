...
// NOTE: previously `export * as Server from "./server"`. esbuild compiles that
// self-namespace re-export into a recursive `await init_server()` inside the
// module's own init body, which deadlocks ESM evaluation. An explicit object
// avoids the self-import cycle.
export const Server = {
  listen,
  openapi,
  get Default() {
