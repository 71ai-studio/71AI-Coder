...
// Lazily construct the runtime so circular import init order doesn't matter:
// at module-load, `Project.Service` may still be undefined under esbuild's
// bundled __esm wrappers; defer the lookup to first use.
let _projectRuntime: ReturnType<typeof makeRuntime<typeof Project.Service, never>> | undefined
const project = {
  runPromise(...args: Parameters<ReturnType<typeof makeRuntime<typeof Project.Service, never>>["runPromise"]>) {
    _projectRuntime ??= makeRuntime(Project.Service, Project.defaultLayer)
    return _projectRuntime.runPromise(...args)
  },
}
